...
export class AdminPermissionService {
  ...
  /**
   * 分页查询权限列表
   * @param {QueryPermDTO} params
   */
  async queryAdminPermission(params: QueryPermDTO) {
  ...
  /**
   * 创建权限
   * @param {CreatePermDTO} params 权限参数
   */
  async createAdminPermission(params: CreatePermDTO) {
  ...
  /**
   * 更新权限
   * @param {UpdatePermDTO} params 更新权限参数
   */
  async updateAdminPermission(params: UpdatePermDTO) {
    const { id, ...columns } = params;
    columns.httpMethod = Array.isArray(columns.httpMethod)
      ? columns.httpMethod.join(',')
      : columns.httpMethod;
